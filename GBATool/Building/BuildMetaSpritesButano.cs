...
public sealed class BuildMetaSpritesButano : Building<BuildMetaSpritesButano>
{
    ...
    protected override async Task<bool> DoGenerate()
    {
        ...
        foreach (FileModelVO item in models)
        {
            ...
            await WriteSpriteFileCpp(outputFileCpp, model, item.Name);
        }
    ...
    private static async Task WriteSpriteFileCpp(StreamWriter outputFile, CharacterModel model, string name)
    {
        ...
        await WriteClassDefinitions(outputFile, model, name);
    }
    ...
    private static async Task WriteClassDeclarations(StreamWriter outputFile, CharacterModel model, string name)
    {
        ...
        await outputFile.WriteLineAsync("    void update();");
        ...
        await outputFile.WriteLineAsync("    bn::vector<bn::sprite_ptr, 32> _currentFrameSprites;");
        ...
        await outputFile.WriteAsync(Environment.NewLine);
        await outputFile.WriteLineAsync("    static const Animation _animation[3];");
        await outputFile.WriteLineAsync("};");
    }

    private static async Task WriteClassDefinitions(StreamWriter outputFile, CharacterModel model, string name)
    {
        string className = name.ToCamelCase();
        ...
        await outputFile.WriteLineAsync("{");
        await outputFile.WriteAsync(Environment.NewLine);
        await outputFile.WriteLineAsync($"const {className}::Animation {className}::_animation[3] = {{ {{4, 3}}, {{4, 3}}, {{4, 3}} }};");
        await outputFile.WriteAsync(Environment.NewLine);
